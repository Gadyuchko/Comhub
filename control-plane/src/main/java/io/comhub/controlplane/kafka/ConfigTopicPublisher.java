 ...
 *
 * <p>{@link #publish(MappingConfig)} writes an upsert record keyed by the config's source topic.
 * {@link #publishTombstone(String)} writes a {@code null}-valued record on the same key so that
...
public interface ConfigTopicPublisher {
    ...
    void publishTombstone(String sourceTopic);
}
